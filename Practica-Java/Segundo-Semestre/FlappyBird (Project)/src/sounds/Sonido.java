            ...
            Thread hilo = new Thread() {
                ...
                public void run() {
                    try {
                        ...
                        Player player;
                        try{
                            fis = new FileInputStream(ruta + "/src/sounds/AudioMuerte.mp3");
                        }catch(FileNotFoundException e){
                            fis = new FileInputStream(ruta + "/complementos/AudioMuerte.mp3");
                        }
                        BufferedInputStream bis = new BufferedInputStream(fis);
                        player = new Player(bis);
                        player.play();
        ...
        Thread hilo = new Thread() {
            ...
            public void run() {
                try {
                    ...
                    Player player;
                    try{
                        fis = new FileInputStream(ruta + "/src/sounds/Yii.mp3");
                    } catch(FileNotFoundException e){
                        fis = new FileInputStream(ruta + "/complementos/Yii.mp3");
                    }
                    BufferedInputStream bis = new BufferedInputStream(fis);
                    player = new Player(bis);
                    player.play();
